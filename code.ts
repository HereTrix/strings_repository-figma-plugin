...
function send(type: MessageType, data: any) {
...
function getTextNodesFrom(node: SceneNode, textNodes: SceneNode[]): SceneNode[] {
  if ('children' in node) {
...
async function updateTextChild(node: SceneNode, name: string, value: string) {
...
// Network
async function pull(items: string[]) {
  if (!credentials) {
    return
  }
...
async function push(items: string[]) {
  if (!credentials) {
    return
  }
...
async function fetchLanguages() {
  ...
  fetch(`${credentials.host}:${credentials.port}/api/plugin/languages`, {
    ...
    .then((response) => {
      if (response && response as string[]) {
